...
$dao->query($sql);
//列出章节
$sql = "select id,biaoti from kecheng_zhangjie where kechengid = {$kechengid} and riqi <= CURRENT_DATE order by biaoti,shunxu";
    ...
    <div class="row" style="text-align: center;background-color: white;">
        ...
        <div class="row" style="background-color: white; padding: 15px;margin-top: 10px;"">
            ...
            <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="get">
                <select name="zhangjieid" style="width: 260px; height: 40px;">
                    <?php if($zhangjieid==-1){
                        echo "<option value='-1' selected>请选择课程:</option>";
                    }else{
                        echo "<option value='-1'>请选择课程:</option>";
                    }
                        ?>
                <?php foreach ($rows as $v):?>
                    ...
                    ?>
                     <option value="<?php echo $v['id'];?>" <?php echo $sel;?>><?php echo $v['biaoti'];?></option>
                <?php endforeach; ?>
                </select>

                <input type="submit" value="确定" style="width: 50px; height: 40px;">
